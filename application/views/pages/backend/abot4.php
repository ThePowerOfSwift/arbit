...
if($active < 25000) {
    $totalGas = 150;
}else if($active >= 25000 && $active < 100000) {
    $totalGas = 250;
...
<div class="container">
    <div class='row margin_Top_50'>
        ...
        <div class='col-sm-4 textAlignCenter pt-4'>
            <span class="">$ = aUSD</span></br>
        </div>
    ...
    <div class="row rowDataBot2">
        ...
        <div class="col-md-4 textAlignCenter">
            <span class="lggFontBOT2">$<?php echo round($gas, 2); ?> / $<?php echo $totalGas; ?> </span></br>
            <span class="lgFontBOT2">Available Gas / Total Gas</span></br>
        </div>
    ...
    <div class="row">
        <div class="col-md-12 textAlignCenter">
            <button class="btn btn-warning" id="shade" data-toggle="modal" data-target="#refillGasTankModal">Refill Gas Tank</button>
        </div>
    </div>
...
<script>
    ...
    function TransferActive(){
        ...
        else if( selectedStopRadio == 100 )
        {
            $("#auctionConfirmationModal").modal("show");
        }
        else {
            $('#apiResponseModal').modal('show');
    ...
    function confirmAccAuction() {
        $("#auctionConfirmationModal").modal("hide");
        $.post( "<?php echo base_url(); ?>abot_active_to_stop_abot_wallet", {stop_percent: "100" })
          .done(function( data ) {
            data = $.parseJSON(data);
            $('#apiResponseDiv').text(data.msg);
            $('#apiResponseModal').modal('show');
    ...
    function refillGasTank(){
        $('#refillGasTankModal').modal('hide');
        var gasTankSelectedVal = $('input[name="gasTankRadio"]:checked').val();
        var SelectAmountGas = 0;
        
        if(gasTankSelectedVal == "full") 
        {
            $.get("<?php echo base_url(); ?>new_system_eth_to_gas", function( data ) {
                data = JSON.parse(data);
                if(data.success == '1') {
                    $('#apiResponseModal').modal('show');
                    $('#apiResponseDiv').html(data.msg);
        ...
        else if(gasTankSelectedVal == "25" || gasTankSelectedVal == "50" || gasTankSelectedVal == "100") 
        {
            $.post( "<?php echo base_url(); ?>system_eth_to_gas_choise", {user_selection:gasTankSelectedVal})
              .done(function( data ) {    
                data = JSON.parse(data);
                if(data.success == '1') {
                    $('#apiResponseModal').modal('show');
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="checkboxWrapper" style="width:100%">
                                <label class="containerRadio">100% (Send to auction)
                                  <input type="radio" name="stopRadio" value="100" onclick="stopAbotCheckPer('100')" />
                                  <span class="radioCheckmark"></span>
                                </label>
                            </div>
                        </div>
            ...
            <script>
                function stopAbotCheckPer(per) {
                    if(per == '1'){
                        ...
                        $('#stopAbotCheckText').html("Your Account will go into Auction.");
                    }
                    else {
...
<div class="modal fade" id="refillGasTankModal">
    <div class="modal-dialog modal-md" role="document">
        <div class="modal-content" style="font-size:12px">
            <div class="modal-header modalHeaderExchange">
                <h5 class="modal-title">Gas Tank</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body pb-4">
                <h5>Select a amount to refill your gas tank.</h5>
                <div class="row">
                    <form>
                        <div class="col-md-12">
                            <div class="checkboxWrapper">
                                <label class="containerRadio" style="width:100%">$25
                                  <input type='radio' name='gasTankRadio' value='25' checked="checked"/>
                                  <span class="radioCheckmark"></span>
                                </label>
                            </div>
                            <div class="checkboxWrapper" style="width:100%">
                                <label class="containerRadio">$50
                                  <input type='radio' name='gasTankRadio' value='50' />
                                  <span class="radioCheckmark"></span>
                                </label>
                            </div>
                            <div class="checkboxWrapper" style="width:100%">
                                <label class="containerRadio">$100
                                  <input type="radio" name="gasTankRadio" value="100" />
                                  <span class="radioCheckmark"></span>
                                </label>
                            </div>
                            <div class="checkboxWrapper" style="width:100%">
                                <label class="containerRadio">Full
                                  <input type="radio" name="gasTankRadio" value="full" />
                                  <span class="radioCheckmark"></span>
                                </label>
                            </div>
                        </div>
            ...
            <div class="modal-footer">
                <button class="btn btn-success" onclick=refillGasTank()>Confirm</button>
                <button class="btn btn-danger" data-dismiss="modal">Cancel</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function confirmReinvest(){
            ...
            if(percent == '25' || percent == '50' || percent == '75' ||percent == '100') {
                $.post( "<?php echo base_url(); ?>auto_reinvest", {percent: percent, lock_reinvest: 0})
                  .done(function( data ) {
                    data = JSON.parse(data);
                    if(data.success == '1')
                    {
...
<div class="modal fade" id="auctionConfirmationModal">
    <div class="modal-dialog modal-md" role="document">
        <div class="modal-content" style="font-size:12px">
            <div class="modal-body">
                <div class="col-md-12">
                    <p>
                        <h5 class="mb-3">Confirming this action will place your aBOT into auction appraisal.</h5>
                        - ETH appraisal deducts 5% of your aBOT balance. If you do not accept the appraisal, 4% of the fee will be returned. (1% non refundable appraisal fee)<br>
                        - ETH appraisal takes up to 48 hours.<br>
                        - ETH appraisal will be your starting bid in auction. (Bids cannot be lower then starting bid)<br>
                        - ETH appraisal will be DELIVERED VIA EMAIL where you will be able approve or deny your appraisal.<br>
                        - Auctions run for 48 hours, if auction is not purchased you can relist 2 times for free.<br>
                        - Highest bid wins the auction.<br>
                        - ETH from winning bid is placed into your main wallet for withdrawal.
                    </p>
                     <div class="col-md-12 mt-4 text-center">
                        <button class="btn btn-success" onclick=confirmAccAuction()>Confirm</button>
                        <button class="btn btn-danger" data-dismiss="modal">Cancel</button>
                    </div>
                </div>
            </div>
