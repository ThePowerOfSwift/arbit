// CONSTANTS
// // THIS CONTRACT IS FOR THE ARB SIDE ONLY // // ALL UNNECESSARY FUNCIONS HAVE  BEEN REMOVED
var contractAddressPROPER = '0x3c52e0a89b554c2ce443bf467050f66Abdd88F3B' // // THis IS  THE ETH SIDE CONTRACT // //
var contractAddress = '0x5eee354e36ac51e9d3f7283005cab0c55f423b23' // // THIS IS THE ETH 
...
window.addEventListener('load', function() {
    ...
    // Withdraw Some Eth click handler - CURRENT //
//    $('#withdraw-some-btn').click(function() {
//        withdrawsomeeth($("#withdraw-eth-amount").val())
 //   })
    ...
    $('#withdraw-some-btn').click(function() 
    {
        let amount = $('#withdraw-eth-amount').val().trim()
        if (amount <= 0 || !isFinite(amount) || amount === '') 
        {
    ...
    // Withdraw ALL - click handler - SIMPLE FUNCTION - ONE CLICK - PULLS ALL USERS FUNDS FROM CONTRACT - NO INPUT FIELD DATA REQUIRED // /
...
function updateData() {
    ...
    if (loggedIn) {
        ...
        contract.ethBalanceOf(currentAddress, function(e, r) {
            ...
            $('.user-hold-balance').text(Number(holdingsAmount.toFixed(2)).toLocaleString() + '')
            contract.calculateEthereumReceived(r, function(e, r) {
                let bal = convertWeiToEth(r)
                $('.user-hold-value').text(bal.toFixed(4) + ' ETH')
...
function attachEvents() {
	...
	// let heyheySound = new Audio('audio/heyheyheey.mp3');
	// heyheySound.volume = 0.65;
	// let hmNoNoSound = new Audio('audio/hmhmnonono.mp3');
  // hmNoNoSound.volume = 0.65
